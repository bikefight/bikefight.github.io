...
(function () {
  ...
  function initAppLogic() {
    ...
    saveNameBtn.addEventListener('click', () => {
      ...
      if (value) {
        ...
        if (currentPosition) {
          ...
          updateMarkerLabel(userId, userName, new Date().toISOString());
        }
      }
    });
    
    // Crosshair icons
    const crossSelf = L.divIcon({ className: 'crosshair-icon self', iconSize: [20, 20], iconAnchor: [10, 10] });
    const crossFriend = L.divIcon({ className: 'crosshair-icon friend', iconSize: [20, 20], iconAnchor: [10, 10] });
    ...
    function updateMarker(id, lat, lng, name) {
        ...
        if (!marker) {
            ...
            markers[id] = marker;
            if (id === userId) {
                marker.bindTooltip(name, { permanent: true, direction: 'top', offset: [0, -18], className: 'marker-label' });
            }
  ...
  if ('geolocation' in navigator) {
    navigator.geolocation.getCurrentPosition(pos => {
      ...
      initAppLogic(); // Initialize the rest of the app
    }, err => {
